...
- Fase 1: parcialmente atendida. A limpeza da venda, limpeza da pesquisa, selecao/rolagem do ultimo item, formula de peso, retorno da quantidade para 1 e protecao contra duplo lancamento possuem implementacao. A edicao de preco de item por peso agora usa o valor final do item. Falta validacao manual do fluxo continuo depois do fechamento.
...
- Fase 6: parcialmente atendida. Contraste, textos longos, notificacoes e divisores redimensionaveis foram ajustados. As barras entre categorias/produtos e produtos/carrinho salvam a proporcao no computador; os controles `-` e `+` do carrinho foram reforcados e os cards de produtos se adaptam a telas estreitas. Falta revisar todos os temas e telas menores.
